...
- **배포 프로세스 (Strict Workflow)**:
  ...
  3. **DB 스키마 검증 (Schema Verification)**: 운영 배포 전 **반드시** `./scripts/pre-deploy-check.sh`를 실행하여 개발/운영 DB 스키마 일치 여부를 확인합니다.
     - 검증 실패 시 운영 DB에 마이그레이션 적용 후 재검증
     - 검증 통과 시에만 운영 배포 진행
  4. **운영 배포 (Production Deployment)**: 개발 서버에서 정상 작동이 확인되고 DB 스키마 검증을 통과한 후, **사용자의 승인**을 받아 `main` 브랜치로 병합합니다.
- **에이전트 행동 규칙**:
