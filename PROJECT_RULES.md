...
- **긴급 핫픽스 프로세스 (Hotfix Workflow)**:
  ...
  2. **main 브랜치로 전환**: 원격 최신 상태로 업데이트합니다.
     ...
     ```
  3. **긴급 수정**: 운영 서버의 심각한 버그를 수정합니다.
  4. **즉시 배포**: 테스트 후 바로 `main` 브랜치에 커밋 및 푸시하여 운영 서버에 배포합니다.
     ```bash
     git add .
     git commit -m "hotfix: 긴급 수정 내용"
     git push origin main
     ```
  5. **역병합 (Backport)**: 배포 완료 후 **반드시** `dev` 브랜치로 역병합하여 동기화합니다.
     ```bash
     git checkout dev
     git merge main  # 충돌 발생 시 핫픽스 내용 우선
     git push origin dev
     ```
  6. **버전 관리**: 핫픽스는 Patch 버전을 올립니다 (예: v1.2.8 → v1.2.9).
