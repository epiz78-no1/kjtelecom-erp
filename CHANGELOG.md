...
## v1.2.35 (2026-01-20) - 일반자재 사용등록 개선 및 광케이블 필터 강화
 
 ### ✨ Features & UI/UX Improvements
 - **일반자재 사용등록 검색 기능(Combobox) 적용**:
   - `TeamMaterialUsage` 화면에서 자재 선택 시, 기존의 단순 드롭다운 대신 검색 가능한 콤보박스(`TeamInventorySelector`)를 적용.
   - 품명 및 규격 검색이 가능해져 수많은 자재 중 원하는 항목을 빠르게 선택 가능.
   - 이미 선택된 항목 제외 및 사업(Division) 필터링 로직이 컴포넌트 내부로 이식되어 UX 일관성 확보.
 - **광케이블 상세 필터 강화**:
   - `OpticalCables` 목록 상단 필터바에 '사업' 필터 추가 (맨 앞 위치).
   - SKT/SKB 등 사업별 재고 현황을 즉시 필터링하여 확인 가능.
...
## v1.2.31 (2026-01-18) - 자재현황 폐기/재사용 처리 개선 및 UI 정렬
 ...
 - **재사용 가능 상태 변경 오류 해결**:
   - `DemolitionMaterials.tsx`에서 '재사용가능' 선택 시 서버로 잘못된 상태 값(`approved_reusable`)을 전송하여 '폐기(`rejected`)'로 처리되던 문제 해결.
   - 전송 값을 `approved`로 수정하여 서버 로직과 일치시킴.
 - **서버 포트 충돌 방지**:
   - macOS 환경(AirPlay Receiver)에서 5000번 포트 점유 시 자동으로 5001번 포트로 우회하도록 확인 및 가이드.

 ### 💄 UI/UX Improvements
 - **자재현황 테이블 개선**:
   - **너비 조정 기능(Resizable Columns)** 도입: `DemolitionIncoming`과 동일한 UX 적용.
   - **컬럼 정리**: 불필요한 원수량/잔량 컬럼 제거, **첨부파일 컬럼** 추가.
   - **다운로드 기능**: 첨부파일이 있는 경우 리스트에서 직접 다운로드 가능.
   - **폐기 사유 입력**: 드롭다운 또는 수정 창에서 '폐기' 선택 시 사유 입력이 필수화됨.
   - **텍스트 정렬**: 품명 등 주요 텍스트 가운데 정렬 적용.
...
### 🐛 Bug Fixes
- **광케이블 사업부 표시 오류 수정 (Data Integrity)**:
  - 현장팀 보유 현황(`FieldOpticalStatus`)에서 케이블의 실제 소속(SKT/SKB) 대신 팀의 소속이 표시되던 문제 해결.
  - 데이터베이스의 잘못된 사업부 데이터('접속팀', '외선팀' 등)를 정규화(`fix_divisions.ts`)하여 데이터 정합성 확보.
- **PC 버전 사용 등록 버튼 미표시 해결**:
  - 모바일에서는 정상적으로 보이던 '사용 등록' 버튼이 PC 버전에서는 관리자(Owner)에게만 보이던 권한 설정 오류 수정.
  - 쓰기 권한(`canWrite`)이 있는 모든 현장팀 작업자가 PC에서도 정상적으로 등록 가능하도록 개선.
- **일괄 등록 권한 강화**:
  - '일괄 등록(Bulk Upload)' 기능은 관리자(Owner) 전용으로 권한 명확히 분리.
...
- **일괄 삭제 시 데이터 정합성 깨짐 해결**:
  - 원인: 일괄 삭제(`Bulk Delete`) 요청 시, 여러 비동기 작업이 동시에 수행되면서 서로 **'삭제 전'**의 옛날 데이터를 읽어 계산하는 **경쟁 조건(Race Condition)** 발생. 이로 인해 삭제는 되었으나 사용량(UsedLength)이 롤백되지 않고 잔존하는 치명적 버그 확인.
  - 해결: **Atomic SQL Update** 적용.
    - 기존: Javascript 레벨에서 `기존값 - 삭제량` 계산 후 DB에 `UPDATE ... SET used = 50` 형태로 전송 (비안전).
    - 변경: DB 레벨에서 `UPDATE ... SET used = GREATEST(0, used - ?)` 형태로 전송하여 **동시성 보장**.
  - 효과: 아무리 많은 로그를 동시에 삭제해도 DB가 순차적으로 정확하게 차감 처리.
